...
#my $remote=`w -i | grep -v ' :0' | awk '/screen -x irc/ {print \$3}'`;
#if ("$remote") { chomp($remote); $con="ssh $remote"; }
#Irssi::settings_add_str("lookandfeel", "beep_cmd", "remote=`w -i | grep -v ' :0' | awk '/screen -x irc/ {print \$3}'`; [ -n \$remote ] && con=`echo ssh -o LogLevel=quiet \$remote | tr -d '\r\n'` || con='' ; $con aplay -q ~/.irssi/scripts/ring.wav &");
#Irssi::settings_add_str("lookandfeel", "beep_cmd", "$con aplay -q ~/.irssi/scripts/ring.wav &> /dev/null &");
Irssi::settings_add_str("lookandfeel", "beep_cmd", "`/home/muxm/bin/get_irc_addr.sh xmu` aplay -q /home/xmu/Music/ring.wav &> /dev/null &");
